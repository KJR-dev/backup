	...
	<script type="text/javascript">
		...
		$
				.ajax({
					...
					type : "post",
					data : {
						action : "get_All_Pending_Complains_engineer"
					},
					dataType : "json",
							...
							$("#tbl-viewcomplaints")
									.append(
											'<tbody><tr data-widget="expandable-table" aria-expanded="false"><td><center>'
													...
													+ '</center></td><td td class="btn btn-secondary my-2"><center>'
													+ data[index].Complaint_Status_Name
													+ '</center></td><td><center>'
													+ '<button class="btn btn-primary" value="'+data[index].Token+'" id="makeProcess" onclick="makeProcess()" >Make Process</button></center></td></tr>'
		...
		function makeProcess() {
			var x = document.getElementById("makeProcess").value;
			/* alert(x); */
			  $.ajax({
				  url : "../Engineer",
				  type : "post",
				  data : {action : "makeProcess" , pincode : x},
				  dataType : "json",
				  success : function(data,textStatus,jqXHR){
					  swal({
						    title: "Done!",
						    text: "Your Task Is Processes!",
						    type: "success"
						}).then(function() {
